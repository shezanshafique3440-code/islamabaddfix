...
let phoneCounter = 0;
const phone = () => `+92300${String(Date.now()).slice(-5)}${String(phoneCounter++).padStart(2, '0')}`;
...
check('   address belongs to this customer',
...
// --- 3. CSRF is actually enforced -----------------------------------------
{
  ...
  check('3. an unauthenticated write is refused',
...
check('   it says which engine answered', r.json.data?.source === 'rules',
...
check('5. a hazard forces EMERGENCY and shows safety guidance',
  r.json.data?.urgency === 'EMERGENCY' && /breaker band/i.test(r.json.data?.safetyNotice ?? ''));
check('   the guidance is not replaced by a generic referral',
...
r = await provider.call('/api/provider/onboarding', {
  ...
  body: {
    ...
    zoneIds: [zone.id],
    availability: [1, 2, 3, 4, 5, 6].map((d) => ({ dayOfWeek: d, startMinute: 540, endMinute: 1140 })),
...
if (r.status >= 400) { await db.$disconnect(); process.exit(1); }
const providerRow = await db.providerProfile.findUniqueOrThrow({ where: { userId: providerUserId } });
check('   status is PENDING_VERIFICATION, not verified on submission',
  providerRow.status === 'PENDING_VERIFICATION');
check('   the full IBAN was never stored',
  providerRow.bankIbanHash?.length === 64 && providerRow.bankAccountLast4 === '6702' &&
...
check('7. unverified provider is not in the public directory',
  (r.json.data ?? []).length === 0, `${(r.json.data ?? []).length} results`);
...
// --- 8. admin approves ----------------------------------------------------
const adminUser = await db.user.findFirstOrThrow({ where: { role: { in: ['ADMIN', 'SUPER_ADMIN'] } } });
r = await admin.call('/api/auth/login', {
  method: 'POST',
  body: { email: adminUser.email, password: process.env.SEED_ADMIN_PASSWORD ?? 'ChangeMe!Admin123' },
});
...
check('   admin approves the provider', r.status === 200,
  describe(r));
if (r.status >= 400) { await db.$disconnect(); process.exit(1); }
const approved = await db.providerProfile.findUniqueOrThrow({ where: { id: providerRow.id } });
check('   status is VERIFIED with a timestamp', approved.status === 'VERIFIED' && approved.verifiedAt !== null);
const badges = await db.providerVerification.findMany({
  where: { providerId: providerRow.id, status: 'APPROVED' }, select: { kind: true },
});
check('   exactly one badge — the onboarding review, nothing implied',
...
check('   the chosen provider was offered the job, not forced to take it',
  ['PENDING', 'PROVIDER_NOTIFIED', 'ACCEPTED'].includes(booking.status) &&
  booking.providerId === providerRow.id,
  booking.status);
check('   the client-sent totals were ignored',
  booking.approvedTotalPaisa === null && booking.finalTotalPaisa === null && booking.commissionPaisa === null);
check('   a status-history row exists', (await db.bookingStatusHistory.count({ where: { bookingId } })) >= 1);
...
check('    status is ACCEPTED with a timestamp', booking.status === 'ACCEPTED' && booking.acceptedAt !== null);

r = await provider.call(`/api/provider/jobs/${bookingId}`);
check('    now that they accepted, the address is released',
  typeof r.json.data?.address?.addressLine === 'string' && r.json.data.address.addressLine.includes('Street 9'));
...
check('    the total is summed server-side: Rs. 3,300', quote.subtotalPaisa === 330_000,
  `${quote.subtotalPaisa} paisa`);
booking = await db.booking.findUniqueOrThrow({ where: { id: bookingId } });
check('    booking is QUOTE_PENDING and still has no agreed price',
...
// --- 12. only the right customer may decide it ---------------------------
{
  ...
  await stranger.call('/api/auth/register', {
    method: 'POST',
    body: {
      fullName: 'Random Person', email: `random.${stamp}@example.pk`,
      phone: phone(), password: 'Rand0m!Pass2026', acceptedTerms: true,
    },
  });
  const res = await stranger.call(`/api/bookings/${bookingId}/quotes`, {
    method: 'POST', body: { quoteId, decision: 'approve' },
  });
  ...
  check('    and they cannot even read the booking', detail.status === 404, `HTTP ${detail.status}`);
}
...
for (const [action, expected] of [
  ['confirm_schedule', 'SCHEDULED'], ['on_the_way', 'ON_THE_WAY'],
  ...
  check(`14. ${action} -> ${expected}`, res.status === 200 && row.status === expected,
...
// --- 15. an illegal transition is refused --------------------------------
{
  ...
  check('15. an illegal transition is refused', res.status >= 400,
    describe(res));
}

// --- 16. completion ---------------------------------------------------------
r = await provider.call(`/api/provider/jobs/${bookingId}/status`, {
  method: 'POST', body: { action: 'complete', notes: 'Gas refill done, leak test clear.' },
});
check('16. provider completes the job', r.status === 200, describe(r));
booking = await db.booking.findUniqueOrThrow({ where: { id: bookingId } });
check('    final total is Rs. 3,300', booking.finalTotalPaisa === 330_000);
check('    commission is 10% of it: Rs. 330', booking.commissionPaisa === 33_000,
...
check('    gross is conserved',
  booking.commissionPaisa + booking.providerEarningsPaisa === booking.finalTotalPaisa);
check('    the guarantee window is frozen onto the booking',
...
check('17. the customer never sees the commission', r.json.data?.pricing?.commissionPaisa === undefined);
r = await provider.call(`/api/provider/jobs/${bookingId}`);
check('    the provider sees their earnings but not the commission',
...
check('    the amount came from the server-side total, not the request',
  payment.amountPaisa === 330_000);
check('    no card data anywhere on the row',
  !/cardNumber|cvv|pan/i.test(JSON.stringify(payment)));

// --- 19. online payment is honest about not being configured -------------
{
  const res = await customer.call(`/api/bookings/${bookingId}/payment`, {
    method: 'POST', body: { method: 'ONLINE_GATEWAY' },
  });
  check('19. online payment says it is not configured, it does not fake success',
...
check("    the provider's rating aggregate was recomputed",
...
const history = await db.bookingStatusHistory.findMany({
  where: { bookingId }, orderBy: { createdAt: 'asc' }, select: { toStatus: true },
});
const sequence = history.map((h) => h.toStatus).join(' → ');
check('21. every transition was recorded',
  sequence.endsWith('SCHEDULED → ON_THE_WAY → ARRIVED → IN_PROGRESS → COMPLETED'), sequence);
...
check('22. a guarantee claim inside the window is accepted', r.status === 201 || r.status === 200,
  describe(r));
check('    the claim is in the database awaiting review',
